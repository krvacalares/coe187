...
class AnimalDataset(Dataset):
    ...
    label_to_folder_map = {label: label + 's' for label in labels}

    def __init__(self, root_dir, d_type, transform=None,
                 resize_size=(128, 128), augment_data=False):
        self.root_dir = root_dir
        self.data_dir = os.path.join(root_dir, 'animals', d_type)
    ...
    def __print_download_manual(self):
        ...
        print("1. Prepare your dataset with folders named: cats, dogs, elephants, horses, lions")
        print("2. Place them into 'data/animals/' folder like so:")
        print("   data/animals/train/cats/")
        print("   data/animals/train/dogs/")
        print("   data/animals/train/elephants/")
        print("   data/animals/train/horses/")
        print("   data/animals/train/lions/")
        print("   data/animals/test/cats/")
        print("   data/animals/test/dogs/")
        print("   data/animals/test/elephants/")
        print("   data/animals/test/horses/")
        print("   data/animals/test/lions/")
        print("******************************************")
...
def get_animal_dataset(data, load_train, load_test):
    """
    ...
    if load_train:
        train_dataset = AnimalDataset(root_dir=data_dir, d_type='train', transform=transform, augment_data=True)
    if load_test:
        test_dataset = AnimalDataset(root_dir=data_dir, d_type='test', transform=transform, augment_data=False)
...
datasets = [
    {
        'name': 'animals',
        'input': (3, 128, 128),
        'output': ('cats', 'dogs', 'elephants', 'horses', 'lions'),
        'loader': get_animal_dataset,
    },
]
